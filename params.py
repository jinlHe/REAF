...
def parse_args(args):
    ...
    parser.add_argument(
        "--zeroshot-template", type=str, default='volume', help="Zero-shot with organ prompt template or volume prompt template."
    )
    parser.add_argument(
        "--input-data-type",
        type=str,
        choices=["stage1", "stage2", "ori"],
        default="stage2",
        help="Data processing type: 'stage1' for uncompressed PT files, 'stage2' for compressed GZ files, 'ori' for processing from original NIfTI files."
    )
    parser.add_argument(
        "--report-root",
        type=str,
        default="",
        help="Path to JSON file containing pre-generated reports. If empty, reports will be generated from abnormality labels."
    )
    parser.add_argument(
        "--fusion-method",
        type=str,
        default="weighted_mean",
        choices=["mean", "max", "weighted_mean", "inverse_weighted_mean", "random_weighted_mean"],
        help="Attention fusion method for combining multi-layer attention weights. "
             "Options: 'mean' (simple average), 'max' (maximum), 'weighted_mean' (deeper layers have higher weights), "
             "'inverse_weighted_mean' (shallower layers have higher weights), 'random_weighted_mean' (random weights)."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="path to checkpoint to load before training (default: none)",
    )
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="vit_base_singlescan_h2_token2744",
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help="Use LoRA for the visual tower.",
    )
    parser.add_argument(
        "--lora-r",
        type=int,
        default=8,
        help="LoRA rank parameter. Lower values use less memory but may reduce performance.",
    )
    parser.add_argument(
        "--lora-alpha",
        type=int,
    ...
    parser.add_argument(
        "--lora-visual-last-n-blocks",
        type=int,
        default=6,
        help="Apply LoRA only to the last N transformer blocks in visual encoder (fallback strategy).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help="Path to CXR-BERT model directory or HuggingFace model name."
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If true, set wandb to offline mode.",
    )
    parser.add_argument(
        "--debug",
        default=False,
